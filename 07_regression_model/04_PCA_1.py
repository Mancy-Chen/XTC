...
# ROI-wise PCA analyses for:
# 1) shape-only
# 2) firstorder-only
# 3) non-volume radiomics
#
# Goal:
# Test whether radiomics can support structural change before/after XTC use
...
output_root = "/data/projects/CSC/code/XTC/07_regression_model/Output/roi_pca_structural_change"
...
def feature_matches_keywords(base_name: str, keywords):
    return any(k in base_name for k in keywords)
...
def pca_scores_from_prepost(df_in, roi_feature_bases):
...
def find_volume_columns_for_roi(df_in, roi_name):
    candidates = []
    for col in df_in.columns:
        if col.startswith(roi_name + "_") and (col.endswith("_pre") or col.endswith("_delta")):
...
for analysis_name, cfg in analysis_configs.items():
    ...
    for roi in target_rois:
        roi_bases = get_roi_feature_bases(roi, feature_bases)
        print(f"{roi}: {len(roi_bases)} feature bases")

        if len(roi_bases) < 2:
            print(f"Skipping {roi}: not enough features.")
            continue

        roi_pc_df, loadings_df, explained_var = pca_scores_from_prepost(df, roi_bases)

        if roi_pc_df is None:
            print(f"Skipping {roi}: not enough complete cases.")
            continue

        roi_pc_df["roi"] = roi
        all_roi_pc_dfs.append(roi_pc_df)

        loadings_df.to_csv(
            os.path.join(output_dir, f"{roi}_PC1_loadings.csv"),
            index=False
        )
        ...
        explained_variance_rows.append({
            ...
            "roi": roi,
            "n_subjects": roi_pc_df.shape[0],
            "n_features_in_pca": len(roi_bases),
            "PC1_explained_variance_ratio": explained_var,
        })

        # paired test
        pre = roi_pc_df["PC1_pre"].to_numpy(dtype=float)
        post = roi_pc_df["PC1_post"].to_numpy(dtype=float)
        delta = roi_pc_df["PC1_delta"].to_numpy(dtype=float)
        ...
        sub_corr = roi_pc_df[["PC1_delta", outcome_col]].dropna().copy()
        ...
        # delta PC1 vs delta volume
        vol_pre_col, vol_delta_col = find_volume_columns_for_roi(df, roi)
        if vol_delta_col is not None:
            tmp = roi_pc_df[["cohort", "id", "PC1_delta"]].merge(
    ...
    if len(all_roi_pc_dfs) == 0:
        print(f"No ROI PCA results for {analysis_name}.")
        continue

    # -------------------------
    # save ROI PC scores
    # -------------------------
    all_roi_pc_df = pd.concat(all_roi_pc_dfs, axis=0, ignore_index=True)
    all_roi_pc_df.to_csv(os.path.join(output_dir, "roi_pc_scores_long.csv"), index=False)

    roi_pc_wide = all_roi_pc_df.pivot_table(
        index=["cohort", "id", outcome_col],
        columns="roi",
        values=["PC1_pre", "PC1_post", "PC1_delta"]
    )
    roi_pc_wide.columns = [f"{metric}_{roi}" for metric, roi in roi_pc_wide.columns]
    roi_pc_wide = roi_pc_wide.reset_index()
    roi_pc_wide.to_csv(os.path.join(output_dir, "roi_pc_scores_wide.csv"), index=False)
    ...
    paired_df.to_csv(os.path.join(output_dir, "roi_pc1_paired_tests.csv"), index=False)

    # -------------------------
    # explained variance
    # -------------------------
    ev_df = pd.DataFrame(explained_variance_rows)
    ev_df.to_csv(os.path.join(output_dir, "roi_pc1_explained_variance.csv"), index=False)
    ...
    corr_df.to_csv(os.path.join(output_dir, "roi_pc1_vs_cognition.csv"), index=False)
    ...
    volcorr_df.to_csv(os.path.join(output_dir, "roi_pc1_vs_volume_change.csv"), index=False)
